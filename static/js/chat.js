...
function loadLogs() {
  $.ajax({
    ...
    success: function (data){
      // console.log(JSON.parse(JSON.parse(JSON.stringify(
        $('#output').val(data.replaceAll("\\n","\n"));
...
let loader = setInterval(loadLogs, 500);
...
function send() {
  $.post( "/getvar@" + channel, {
    javascript_data: document.getElementById("result").value
});
document.getElementById("result").value = "";
}
