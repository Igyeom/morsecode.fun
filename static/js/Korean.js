...
var decodeEng = function(morseCode){
  var words = (morseCode).split('  ');
  var letters = words.map((w) => w.split(' '));
  var decoded = [];
  ...
  for(var i = 0; i < letters.length; i++){
    decoded[i] = [];
    for(var x = 0; x < letters[i].length; x++){
        if(KOREAN[letters[i][x]]){
            decoded[i].push( KOREAN[letters[i][x]] );
        }
    }
  }
...
function dit_or_dah() {
  if (ontime < 1.2 / wpm * 100) {
    ...
    document.getElementById('result').value += decodeEng(decodeMorse(ditdah));
    console.log(decodeEng(decodeMorse(ditdah)));
  } else {
    ditdah += "-";
    document.getElementById('result').value = document.getElementById('result').value.slice(0, -1)
    document.getElementById('result').value += decodeEng(decodeMorse(ditdah));
